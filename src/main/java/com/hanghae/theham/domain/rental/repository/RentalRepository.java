...
public interface RentalRepository extends JpaRepository<Rental, Long> {
    ...
    @Query(value =
            "SELECT *, r.distance FROM " +
                    ...
                    "FROM rental_tbl WHERE is_deleted = false) as r " +
                    "WHERE r.distance < 4 " +
    ...
    @Query(value =
            "SELECT *, r.distance FROM " +
                    "(SELECT *, ST_DISTANCE_SPHERE(POINT(:userLongitude, :userLatitude), POINT(rental_tbl.longitude, rental_tbl.latitude)) / 1000 as distance " +
                    "FROM rental_tbl WHERE is_deleted = false) as r " +
                    "WHERE r.distance < 4 AND category=:category " +
                    "ORDER BY r.created_at DESC " +
                    "LIMIT :limit OFFSET :offset",
            nativeQuery = true)
    ...
    @Query(value =
            "SELECT * FROM rental_tbl " +
                    ...
                    "id in (SELECT DISTINCT rental_id FROM rental_image_tbl WHERE title LIKE CONCAT('%', :keyword, '%')) " +
    ...
    @Query(value =
            "SELECT count(*) AS distance FROM " +
                    ...
                    "WHERE (id IN (SELECT DISTINCT rental_id FROM rental_image_tbl WHERE title LIKE CONCAT('%', :keyword, '%')) " +
                    "OR content LIKE CONCAT('%', :keyword, '%')) " +
                    "AND ST_DISTANCE_SPHERE(POINT(:userLongitude, :userLatitude), POINT(rental_tbl.longitude, rental_tbl.latitude)) / 1000 < 4 " +
                    "AND is_deleted = false) AS r"
            , nativeQuery = true)
